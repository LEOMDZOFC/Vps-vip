...
import zipfile
import psutil
import hashlib                         # ← FIX: import adicionado
from functools import wraps            # ← FIX: import adicionado
from pathlib import Path               # ← FIX: import adicionado (para DATA_FILE)
...
DATA_FILE = Path('data.json')          # ← FIX: DATA_FILE definido como Path
BOTS_DIR = 'bots'
CPU_HISTORY = {}
CRASH_COUNT = {}
NET_STATS = {}
...
# ============================================
# Data helpers (consolidado com Path)
# ============================================
def load_data():
    if DATA_FILE.exists():
        try:
...
# ← FIX: Decorator admin_required adicionado
...
def api_stats(server_id):
    ...
    return jsonify({'cpu': cpu, 'ram': ram, 'uptime': uptime, 'net_in': net_in, 'net_out': net_out,
                    'cpu_limit': server.get('cpu_limit', 80), 'status': server.get('status', 'stopped')})
...
# ============================================
# 🔥 GITHUB API (Git-less Python Download)
# ============================================
